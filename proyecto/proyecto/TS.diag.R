...
TS.diag = function(X, col = "gray", border = "darkblue", breaks = seq(-10,10,1)){
par(mfrow = c(2,3), bty = "n", las = 1)
Box.Ljung.Test(X, lag = 24, col = col, border = border)
axis(1, 0:24)
aux = acf(X, plot = F, lag.max = 24, na.action = na.pass)$acf[,,1]
plot(aux ~ c(0:24), col = col, lwd = 2, xaxt = "n", main = expression("Auto Correlation Function"), las = 1, ylim = c(-1,1), type = "h", xlab = "Lag")
